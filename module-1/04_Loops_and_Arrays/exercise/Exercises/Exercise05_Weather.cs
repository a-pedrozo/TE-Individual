...
namespace Exercises
{
    public class Exercise05_Weather
    {
        ...
        */
        ...
        { int freezingTemp = 32;
          int numOfFreezingDays = 0;
            for( int i = 0; i < dailyHighs.Length; i++)
            {
                if(dailyHighs[i] <= freezingTemp)
                {
                    numOfFreezingDays++;
                }
            }
            return numOfFreezingDays;
        }

        /*
        GaleForce also needs to determine the hottest day when given an upcoming forecast.  solution is finding the highest number looping through 
        ...
        */
        public int HottestDay(int[] dailyHighs)
        {
            ...
            for (int i = 0; i < dailyHighs.Length; i++)
            {
                int hotday = dailyHighs[i];
                if(hotday > hottestDay)
                {
                    hottestDay = hotday;
                }
            }
        ...
        Examples:
        FixTemperatures([33, 30, 32, 37, 44, 31, 41]) → [35, 30, 34, 37, 46, 31, 43]      for loop looking at every other value 
