...
namespace DadJokesServer.DAOs
{
    public class JokeDAO : IJokeDAO
    {
        ...
        {
            const string sql = "SELECT TOP 1 j.date_added, j.id, j.punchline, j.setup, j.user_id " +
                "FROM Jokes j WHERE j.id = @id";
        ...
        public List<Joke> GetAllDadJokes()
        {
            ...
            const string sql = "SELECT j.date_added, j.id, j.punchline, j.setup, j.user_id " +
                "FROM Jokes j ORDER BY date_added";
