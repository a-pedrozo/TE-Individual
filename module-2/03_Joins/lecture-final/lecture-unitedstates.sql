...
SELECT
	c.city_name,
	c.state_abbreviation -- This is a Foreign Key to state.state_abbreviation
FROM
	city c
WHERE
...
FROM
	city c
	INNER JOIN state s ON s.state_abbreviation = c.state_abbreviation
	--INNER JOIN state s ON s.capital = c.city_id -- Will filter down to just capital cities
WHERE
...
-- Write a query to retrieve the names and populations of all the cities in the Midwest census region - left outer joining by capital city.
SELECT
	c.city_name,
	c.population,
	s.state_name AS 'City is Capital of this state',
	s.census_region -- Only present where we could join from city to state on capital city (only for capital cities)
FROM
	city c
	LEFT OUTER JOIN state s ON s.capital = c.city_id -- Left outer join does NOT filter, but may result in NULL columns
...
FROM
	city c
WHERE
	c.city_name LIKE 'W%' -- starts with a W
