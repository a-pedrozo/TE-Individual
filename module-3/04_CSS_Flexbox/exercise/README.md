...
`folio-exercise` gives you the chance to test your Flexbox knowledge by building a complete page, using CSS Flexbox to style a photography portfolio.

_Note: there is an **optional** `optional-exercises` folder with many small exercises. You can do these for additional practice, but they will not be graded.
...
The portfolio exercise can be found inside the `folio-exercise` directory.
...
# Optional-Exercises: Practice with flexbox properties

`optional-exercises` has a collection of exercises that test your knowledge of flex container and flex item properties.

## Getting started

1. In VS Code, navigate to the `optional-exercises` folder.
